import React from 'react';
import { Link, usePage } from '@inertiajs/react';
import {
    LayoutDashboard,
    Globe2,
    Activity,
    BookOpen,
    LogOut,
    UserCircle,
    ChevronRight,
    Menu,
    X,
    Users,
    Megaphone,
    Image as ImageIcon,
    Video,
    Settings,
    Mail
} from 'lucide-react';

interface NavItem {
    name: string;
    icon: React.ElementType;
    href: string;
    active: boolean;
}

interface AdminSidebarProps {
    isOpen: boolean;
    setIsOpen: (open: boolean) => void;
}

export function AdminSidebar({ isOpen, setIsOpen }: AdminSidebarProps) {
    const { props } = usePage();
    const user = props.auth?.user;

    const can = props.auth?.can || {};

    const canManageSettings = user?.role === 'admin';

    const navItems = [
        {
            name: 'Dashboard',
            icon: LayoutDashboard,
            href: route('dashboard'),
            active: route().current('dashboard'),
            show: true
        },
        {
            name: 'Partners',
            icon: Globe2,
            href: route('admin.partners.index'),
            active: route().current('admin.partners.*'),
            show: can.view_partners
        },
        {
            name: 'Services & Solutions',
            icon: Activity,
            href: route('admin.services.index'),
            active: route().current('admin.services.*'),
            show: can.view_services
        },
        {
            name: 'Medical Depts',
            icon: BookOpen,
            href: route('admin.departments.index'),
            active: route().current('admin.departments.*'),
            show: can.view_departments
        },
        {
            name: 'Team Management',
            href: route('admin.users.index'),
            icon: Users,
            active: route().current('admin.users.*'),
            show: can.view_users
        },
        {
            name: 'Promotions',
            href: route('admin.offers.index'),
            icon: Megaphone,
            active: route().current('admin.offers.*'),
            show: can.view_offers
        },
        {
            name: 'Hero Slider',
            href: route('admin.hero-slides.index'),
            icon: ImageIcon,
            active: route().current('admin.hero-slides.*'),
            show: can.view_hero
        },
        {
            name: 'Testimonials',
            icon: Video,
            href: route('admin.testimonials.index'),
            active: route().current('admin.testimonials.*'),
            show: can.view_testimonials
        },
        {
            name: 'Contact Messages',
            icon: Mail,
            href: route('admin.contact-messages.index'),
            active: route().current('admin.contact-messages.*'),
            show: can.view_contact_messages
        },
        {
            name: 'Settings',
            icon: Settings,
            href: route('admin.settings.index'),
            active: route().current('admin.settings.*'),
            show: canManageSettings
        },
    ].filter(item => item.show);

    return (
        <aside
            className={`fixed lg:static inset-y-0 left-0 z-50 bg-white border-r border-slate-200 transition-all duration-300 shadow-xl lg:shadow-none ${isOpen ? 'w-72' : 'w-20'
                } overflow-hidden flex flex-col h-full`}
        >
            {/* Header/Logo */}
            <div className="p-6 flex items-center justify-between border-b border-slate-100 shrink-0">
                <div className={`flex items-center gap-3 transition-opacity duration-300 ${!isOpen && 'lg:opacity-0'}`}>
                    <div className="w-10 h-10 bg-primary rounded-xl flex items-center justify-center text-white shadow-lg shadow-primary/20">
                        <span className="font-black text-xl">T</span>
                    </div>
                    <div className="flex flex-col whitespace-nowrap">
                        <span className="font-black text-slate-900 tracking-tight">TERMA</span>
                        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Admin Panel</span>
                    </div>
                </div>
                <button
                    onClick={() => setIsOpen(!isOpen)}
                    className="p-2 hover:bg-slate-50 rounded-xl transition-colors text-slate-400"
                >
                    {isOpen ? <X className="lg:hidden w-5 h-5" /> : <Menu className="w-5 h-5" />}
                </button>
            </div>

            {/* Navigation */}
            <nav className="flex-1 p-4 space-y-2 overflow-y-auto">
                {navItems.map((item) => (
                    <Link
                        key={item.name}
                        href={item.href}
                        className={`flex items-center gap-4 px-4 py-3 rounded-2xl transition-all group whitespace-nowrap ${item.active
                            ? 'bg-primary text-white shadow-lg shadow-primary/20'
                            : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900'
                            }`}
                    >
                        <item.icon className={`w-5 h-5 shrink-0 ${item.active ? 'text-white' : 'text-slate-400 group-hover:text-primary'}`} />
                        <span className={`font-bold text-sm transition-opacity duration-300 ${!isOpen && 'lg:hidden'}`}>
                            {item.name}
                        </span>
                        {item.active && isOpen && (
                            <ChevronRight className="ml-auto w-4 h-4 opacity-50" />
                        )}
                    </Link>
                ))}
            </nav>

            {/* Footer User Profile */}
            <div className="p-4 border-t border-slate-100 shrink-0">
                <div className={`flex items-center gap-3 p-3 rounded-2xl hover:bg-slate-50 transition-colors cursor-pointer group`}>
                    <div className="w-10 h-10 rounded-xl bg-slate-100 flex items-center justify-center text-slate-500 overflow-hidden shrink-0">
                        {user?.avatar ? <img src={user.avatar} alt="" /> : <UserCircle className="w-6 h-6" />}
                    </div>
                    <div className={`flex flex-col transition-opacity duration-300 ${!isOpen && 'lg:hidden'} overflow-hidden`}>
                        <span className="text-sm font-bold text-slate-900 truncate max-w-[140px]">{user?.name}</span>
                        <span className="text-[10px] text-slate-400 truncate max-w-[140px]">{user?.email}</span>
                    </div>
                    <Link
                        href={route('logout')}
                        method="post"
                        as="button"
                        className={`ml-auto p-2 text-slate-400 hover:text-red-500 transition-opacity duration-300 ${!isOpen && 'lg:hidden'}`}
                    >
                        <LogOut className="w-4 h-4" />
                    </Link>
                </div>
            </div>
        </aside>
    );
}
